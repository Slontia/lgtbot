...
public:
  ...
  static std::pair<ErrCode, const char*> ConnectDB(const std::string addr, const std::string user, const std::string passwd, const std::string db_name, const bool new_db)
  {
    ...
    try
    {
      ...
      if (!driver) { return { EC_UNEXPECTED_ERROR, nullptr }; }
      connection = std::unique_ptr<sql::Connection>(driver->connect(addr, user, passwd));
      if (!connection) { return { EC_UNEXPECTED_ERROR, nullptr }; }
    ...
    catch (sql::SQLException e)
    {
      ...
      case 1045: /* ER_ACCESS_DENIED_ERROR */
        return { EC_DB_CONNECT_DENIED, MakeErrorMsg(e) };
      default:
        return { EC_DB_CONNECT_FAILED, MakeErrorMsg(e) };
      }
    }
    ...
    catch (sql::SQLException e)
    {
      ...
      case 1049: /* ER_BAD_DB_ERROR */
        return { EC_DB_NOT_EXIST, MakeErrorMsg(e) };
      default:
        return { EC_UNEXPECTED_ERROR, MakeErrorMsg(e) };
      }
    }
  ...
  std::optional<uint64_t> GetGameIDWithName(const std::string& game_name)
  {
    ...
    return Transaction::ExecuteTransaction(*connection_, [&](Transaction& trans)
    {
      game_id = trans.ExecuteOneRow<trans.STMT_QUERY>("SELECT game_id FROM game_info WHERE name = \'", game_name, "\'")->getUInt64(1);
      return true;
