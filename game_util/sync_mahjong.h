...
struct SyncMahjongGamePlayer
{
    ...
    std::string PublicDoraHtml() const {
        return DoraHtml(option_.image_path_, false, doras_manager_.Doras(), false);
    }
    ...
    std::string Html(const HtmlMode html_mode) const
    {
        ...
        if (!fu_results_.empty()) {
            s += "<center> <font size=\"6\">\n\n " HTML_COLOR_FONT_HEADER(blue) " **和&nbsp;&nbsp;了** "
                HTML_FONT_TAIL "\n\n</font> </center>\n\n";
            s += DoraHtml(option_.image_path_, IsRiichi(), doras_manager_.Doras(), true);
            s += "\n\n";
