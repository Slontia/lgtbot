...
#define ASSERT_USER_PROFILE(uid, zero_sum, top, match_count, recent_count, achievement_count) \
[&]() -> UserProfile { \
    ...
    EXPECT_EQ((achievement_count), user_profile.recent_achievements_.size()); \
    return user_profile; \
}()
...
TEST_F(TestDB, get_user_profile_one_match)
{
    ...
    const auto profile_3 = ASSERT_USER_PROFILE(UserID("3"), 10, 10, 1, 1, 0);
...
TEST_F(TestDB, get_recent_achievements_from_user_profile)
{
    ...
    ASSERT_USER_PROFILE(UserID("2"), 10, 10, 1, 1, 0);
}
...
TEST_F(TestDB, get_achievement_statisic)
{
    ...
    result = db_manager_->GetAchievementStatistic(UserID("1"), "mygame", "myachievement");
    ASSERT_EQ(2, result.count_);
    ASSERT_EQ(2, result.achieved_user_num_);
    result = db_manager_->GetAchievementStatistic(UserID("2"), "mygame", "myachievement");
    ASSERT_EQ(0, result.count_);
    ASSERT_EQ(2, result.achieved_user_num_);
    result = db_manager_->GetAchievementStatistic(UserID("3"), "mygame", "myachievement");
    ASSERT_EQ(1, result.count_);
    ASSERT_EQ(2, result.achieved_user_num_);
    result = db_manager_->GetAchievementStatistic(UserID("1"), "mygame", "other_achievement");
    ASSERT_EQ(0, result.count_);
