...
inline constexpr uint32_t Count();
template <typename EnumType, typename = typename std::enable_if_t<std::is_enum_v<EnumType>>>
inline const char* ToString(const EnumType e);
template <typename EnumType, typename = typename std::enable_if_t<std::is_enum_v<EnumType>>>
inline const std::array<EnumType, Count<EnumType>()>& Members();
...
#define ENUM_END(name)                                  \
    ...
    ;                                                   \
    template <>                                         \
    constexpr uint32_t Count<name>()                    \
    {                                                   \
        return static_cast<uint32_t>(name::name##_MAX); \
    }
...
#define ENUM_BEGIN(name)                                      \
    template <>                                               \
    inline const char* ToString<name>(const name e)           \
    {                                                         \
...
#define ENUM_BEGIN(name)                                     \
    template <>                                              \
    inline const std::array<name, Count<name>()>& Members()  \
    {                                                        \
...
template <typename EnumType, typename = typename std::enable_if_t<std::is_enum_v<EnumType>>>
inline auto operator<=>(const EnumType e1, const EnumType e2)
{
