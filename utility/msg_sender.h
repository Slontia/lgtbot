...
struct AtMsg { const uint64_t uid_; };
struct UserMsg { const uint64_t uid_; };
struct GroupUserMsg { const uint64_t uid_; const uint64_t gid_; };
struct PlayerMsg { const uint64_t pid_; };
...
class MsgSenderForGame
{
 public:
  ...
  MsgSenderForGame& operator<<(const Arith arith)
  {
...
class MsgSenderForBot
{
 public:
  ...
  template <typename Arith> requires std::is_arithmetic_v<Arith>
  MsgSenderForBot& operator<<(const Arith arith)
  {
    const std::string s(std::to_string(arith));
    String(s.data(), s.size());
    return *this;
...
class MsgSenderWrapper
{
 public:
  ...
  MsgSenderWrapper(Sender* const sender, const Deleter& deleter) : MsgSenderWrapper(std::unique_ptr<Sender, Deleter>(sender, deleter)) {}
  ...
  {
    for (const auto& sender : senders_) { *sender << std::forward<Arg>(arg); }
    return *this;
  }
