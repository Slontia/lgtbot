...
void Question::RunCode(vector<Player> &players, MsgSenderBase& sender)
{
    ...
    {
        var["Selected." + to_string(i)] = 0;
    }
    for(int i = 0; i < players.size();i++)
    {
        var["Selected." + to_string(players[i].nowN)]++;
    }
    mn = 10000, mx = -10000;
    for(int i = 0; i < options.size(); i++)
    {
        mn = min(mn, var["Selected." + to_string(i)]);
        mx = max(mx, var["Selected." + to_string(i)]);
    }
    var["MinSelectedNum"] = mn;
    var["MaxSelectedNum"] = mx;
...
//        sender() << item;
    ...
    {
//        if(!checkLength(sender, line, "var", 2, items.size())) return;
