...
const uint64_t k_multiple = 0;  // the default score multiple for the game, 0 for a testing game, 1
                                // for a formal game, 2 or 3 for a long formal game
...
string GameOption::StatusInfo() const {
  return "共 " + to_string(GET_VALUE(回合数)) + " 回合，每回合超时时间 " +
         to_string(GET_VALUE(时限)) + " 秒";
}
...
class RoundStage : public SubGameStage<> {
 public:
  ...
  string AddKeyboard(string s1, string s2, string g1, string g2, string r1, string r2) {
    ...
    for (int i = 0; i < l; i++) {
      if (g1[i] <= 'z' && g1[i] >= 'a') {
        ...
        if (app == 0) main_stage().player_used_[0][g1[i] - 'a'] = '-';

        if (r1[i] == '2') main_stage().player_used_[0][g1[i] - 'a'] = '2';
      ...
      if (g2[i] <= 'z' && g2[i] >= 'a') {
        ...
        if (app == 0) main_stage().player_used_[1][g2[i] - 'a'] = '-';

        if (r2[i] == '2') main_stage().player_used_[1][g2[i] - 'a'] = '2';
    ...
    for (int i = 0; i < l1 + 4 + l1; i++) ret += AddKeyLetter(-1, 'X');
    ret += "</tr><tr>";
    ...
    ret += "<table style=\"text-align:center;margin:auto;\"><tbody><tr>";
    for (int i = 0; i < l2 + 5 + l2; i++) ret += AddKeyLetter(-1, 'X');
    ret += "</tr><tr>";
    ...
    ret += "<table style=\"text-align:center;margin:auto;\"><tbody><tr>";
    for (int i = 0; i < l3 + 7 + l3; i++) ret += AddKeyLetter(-1, 'X');
    ret += "</tr><tr>";
  ...
  void calc() {
    ...
    Boardcast() << (main_stage().UI + keyboardUI);
 ...
 private:
  AtomReqErrCode Submit_(const PlayerID pid, const bool is_public, MsgSenderBase& reply,
    ...
    // These codes were temporary annotated because I can't find a way to check whether it is a
    // single game.
  ...
  AtomReqErrCode SubmitInternal_(const PlayerID pid, MsgSenderBase& reply,
