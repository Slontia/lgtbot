 ...
 public:
  NumberStage(Game<StageEnum, GameEnv>& game)
    : AtomStage(game, NUMBER_STAGE, "设置数字阶段",
      {
        MakeStageCommand(this, "设置数字", &NumberStage::Number, std::make_unique<ArithChecker<uint8_t, 1, 6>>("数字")),
      })
  {}

 private:
   void Number(const uint64_t pid, const bool is_public, const std::function<void(const std::string&)> reply, const uint8_t num)
   {
...
public:
  LieStage(Game<StageEnum, GameEnv>& game)
    : AtomStage(game, LIE_STAGE, "设置数字阶段",
      {
        MakeStageCommand(this, "提问数字", &LieStage::Lie, std::make_unique<ArithChecker<uint8_t, 1, 6>>("数字")),
      })
  {}

private:
  void Lie(const uint64_t pid, const bool is_public, const std::function<void(const std::string&)> reply, const uint8_t num)
  {
    if (pid != game_.game_env().questioner_)
    {
...
public:
  GuessStage(Game<StageEnum, GameEnv>& game)
    : AtomStage(game, LIE_STAGE, "设置数字阶段",
      {
        MakeStageCommand(this, "提问数字", &GuessStage::Guess, std::make_unique<BoolChecker>("质疑", "相信")),
      })
  {}
...
private:
  void Guess(const uint64_t pid, const bool is_public, const std::function<void(const std::string&)> reply, const bool doubt)
  {
    ...
    game_.Boardcast((std::stringstream()
      << "实际数字为" << game_.game_env().num_ << "，"
      << (doubt ? "怀疑" : "相信") << (suc ? "成功" : "失败") << "，"
      << "玩家" << game_.At(loser) << "获得数字" << game_.game_env().num_
    ).str());
...
class MainStage : public CompStage<StageEnum, GameEnv>
{
 ...
 private:
   bool JudgeOver()
   {
     const std::array<uint8_t, 6>& nums = game_.game_env().player_nums_[game_.game_env().questioner_];
