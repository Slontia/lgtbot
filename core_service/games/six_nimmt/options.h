...
EXTEND_OPTION("游戏模式（可选：单局模式、22/33/66分模式）", 模式, (AlterChecker<uint32_t>({{"单局", 0}, {"22分", 1}, {"33分", 2}, {"66分", 3}})), 0)
EXTEND_OPTION("策略变体：根据游戏人数限制使用的卡片数量", 策略, (BoolChecker("开启", "关闭")), false)
EXTEND_OPTION("专家变体：牌可以被添加到一行的最左边或者最右边", 专家, (BoolChecker("开启", "关闭")), false)
EXTEND_OPTION("大胃王模式：此模式每个牛头变更为加分", 大胃王, (BoolChecker("开启", "关闭")), false)
